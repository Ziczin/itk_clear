...
class CatalogClient:
    ...
    async def check_stock(self, item_id: str, quantity: int):
        logger.info("Client.Catalog.CheckStock")
    ...
    async def send_notification(
        ...
        try:
            async with self.session.post(
                ...
                if response.status not in (200, 201):
                    logger.error("Notification request failed", status=response.status)
            ...
            logger.exception("Notification sending failed", reference_id=reference_id)
